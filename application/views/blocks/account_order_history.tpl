            ...
            <table class="orderitems">
                <tr>
                    ...
                    <td>
				     ...
				     [{assign var=oxPersParams value=$orderitem->getPersParams()}]
				     
				    
                                    [{foreach key=sVar from=$oxPersParams item=aParam}]
				   
                                        [{if $aParam }]
					
					    [{if $oxPersParams.areacalc_active == 1 }]
					    [{if $sVar != 'areacalc_active' }]
						...
						[{if $sVar == 'MaterialTypesSelect' }]
						 ...
						 <br />[{ oxmultilang ident=$sVar suffix="COLON" }] [{ oxmultilang ident="ja" }]
					       ...
					       [{else }]
						    <br />[{ oxmultilang ident=$sVar suffix="COLON" }] [{$aParam}]
					       [{/if}]     
  [{/if}]   
					   [{else }]
